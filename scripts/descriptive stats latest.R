...
#plot test
figure <- ggplot(data=df_used_materials_reshaped, aes(x=as.factor(exam), y=share, group = 1))
figure + geom_line(aes(color = method_used), linewidth = 2) +
  geom_line(aes(linetype = method_used)) +
  scale_x_discrete(limits = groups)
  
myplot <- ggplot(data=df_used_materials_reshaped, aes(x = group, y = share, group = group, colour = group)) +
